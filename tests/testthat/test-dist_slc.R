...
test_that("Errors when one of the four geocoords are non-numeric", {
  ...
  y2 <- 80
  expect_error(
    dist_slc(lat1 = as.character(x1), lon1 = y1, lat2 = x2, lon2 = y2)
  )
  expect_error(
    dist_slc(lat1 = x1, lon1 = as.character(y1), lat2 = x2, lon2 = y2)
  )
  expect_error(
    dist_slc(lat1 = x1, lon1 = y1, lat2 = as.character(x2), lon2 = y2)
  )
  expect_error(
    dist_slc(lat1 = x1, lon1 = y1, lat2 = x2, lon2 = as.character(y2))
  )
})
...
test_that("Returns equivalent value for equivalent deg and rad", {
  ...
  r3 <- to_rad(d3)
  r4 <- to_rad(d4)
  dist_deg <- dist_slc(d1, d2, d3, d4, type = "deg")
  dist_rad <- dist_slc(r1, r2, r3, r4, type = "rad")
